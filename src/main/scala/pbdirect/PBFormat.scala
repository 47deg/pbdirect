...
sealed trait PBFormat[A] extends PBScalarValueReader[A] with PBFieldWriter[A]
...
object PBFormat extends PBFormatImplicits {
  def apply[A](implicit reader: PBScalarValueReader[A], writer: PBFieldWriter[A]): PBFormat[A] =
    new PBFormat[A] {
